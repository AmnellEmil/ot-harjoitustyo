...
def send_result_to_sheets(name, time, rows, columns, bombs):
    ...
    """
    gcc = pygsheets.authorize(service_file="ot-harjoitustyo-035eb11cf15a.json")
    sht = gcc.open("Ot-harjoitustyo High Scores")
    wks = sht[0]
    sheet = wks.get_as_df()
    preset = "Custom"
    ...
    if name == "":
        name = "Anonymous"
    entry = pd.Series([name, time, rows, columns, bombs,
                      preset], index=sheet.columns)
