...
class Button():
    ...
    def draw(self):
        if self.hover:
            ...
            self.text_surf,self.text_rect=draw_text(self.text,self.font,"Black",self.x,self.y)
            self.screen.blit(self.text_surf,self.text_rect)
        else:
            pg.draw.rect(screen, "Black", self.rect)
            self.text_surf,self.text_rect=draw_text(self.text,self.font,"White",self.x,self.y)
            self.screen.blit(self.text_surf,self.text_rect)
...
def main_menu(screen):
    ...
    controls_button=Button(screen,"Controls",font,200,100,disp_w/2,disp_h/2+110)
    escape_button=Button(screen,"Escape: go back",font,500,100,disp_w/2,100)
    r_button=Button(screen,"R: restart game",font,500,100,disp_w/2,250)
    m1_button=Button(screen,"Mouse 1: reveal square",font,500,100,disp_w/2,400)
    m2_button=Button(screen,"Mouse 2: flag square",font,500,100,disp_w/2,550)
    ...
    while True:
        ...
        if options_interface:
            ...
            draw_text_on_screen(screen,"Presets:",font,"Black",disp_w*(1/6),100)
            easy_button.draw()
            ...
            draw_text_on_screen(screen, "Manual:", font, "Black", disp_w/2, 100)
...
def options(screen):
    ...
    medium_s, medium_r = draw_text("Medium", font, "Black", disp_w/4, disp_h/2+50)
    ...
    custom_s, custom_r = draw_text("Custom", font, "Black", disp_w/2, disp_h/2+50)
    pop_s,pop_r,message_s,message_r=draw_popup("This message should not be seen")
    popup_active=False
    rows_active = False
    time=10
    rows_text = "rows"
    rows_s, rows_r = draw_text(rows_text, font, "Black", disp_w/4, disp_h*(4/5))
    while running:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                pg.quit()
                ...
                if rows_r.collidepoint(event.pos):
                    ...
                    if rows_active:
                        if rows_text.isnumeric():
                            ...
                            pop_s,pop_r,message_s,message_r=draw_popup("Rows must be an integer")
                            popup_active=True
                            time=perf_counter()
                        rows_text = "rows"
                        rows_active = False
        ...
        info_s, info_r = draw_text(f"Rows: {rows}, Columns: {columns}, Bombs: {nbombs}", font, "Black", disp_w/2, 100)
        rows_s, rows_r = draw_text(rows_text, font, "Black", disp_w/4, disp_h*(4/5))
        trows_s, trows_r = draw_text("Rows", font, "Black", disp_w/4, disp_h*(4/5)-50)
        screen.fill("Grey")
...
def minesweeper(screen, rows, columns, nbombs):
    ...
    screen = pg.display.set_mode(((columns)*(pixels+1), (rows)*(pixels+1)), pg.RESIZABLE)
    while running:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                pg.quit()
